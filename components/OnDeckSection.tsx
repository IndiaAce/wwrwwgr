...
export default function OnDeckSection({ items, loading, onEdit }: OnDeckSectionProps) {
  if (loading) {
    return (
      <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-6 gap-3 p-4 md:p-6">
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center px-8 py-32 text-center gap-3">
        <List size={52} className="opacity-20 text-ink" />
        ...
        return (
          <section key={group.label} className="mb-8">
            <div className="flex items-center gap-2 px-4 md:px-6 py-3">
              ...
              <span className="text-ink-faint text-sm">({group.items.length})</span>
            </div>
            <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-6 gap-3 px-4 md:px-6">
              {group.items.map(item => <DeckCard key={item.id} item={item} onEdit={onEdit} />)}
            </div>
          </section>
        )
      })}
...
function DeckCard({ item, onEdit }: { item: Item; onEdit: (item: Item) => void }) {
  ...
  return (
    ...
    >
      <div className="relative w-full aspect-[2/3] rounded-xl overflow-hidden bg-surface shadow-lg group-active:scale-95 group-hover:ring-1 group-hover:ring-accent/30 transition-all">
      ...
      <p className="text-xs text-ink leading-tight line-clamp-2 px-0.5">{item.title}</p>
      {item.creator && <p className="text-[10px] text-ink-muted truncate px-0.5 -mt-1">{item.creator}</p>}
    </button>
  )
}
